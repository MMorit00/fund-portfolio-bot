...
1. **单 AI + Tools 模式**（不做多 Agent）
   - 先跑通单 AI，验证可行性
   - 多 Agent 编排留到 v1.x（可选）

2. **不为 AI 改数据库 Schema**
   - ActionLog 是 AI 的唯一信息源
   - 需要金额等信息时，代码层 JOIN trades 拼装
   - 内存中组装数据，保持数据层干净
...
**关键澄清**：
- BillFacts/DcaFacts 是导入时的临时分析工具，不是 AI 的主要输入
- ActionLog 才是 AI 的信息源，记录了用户的决策行为
- AI 通过 Tools 获取 ActionLog + 关联数据（trades、fund_restrictions）

**目录规划**（待实现）：
```
src/ai/
  ├─ client/           # LLM API 客户端（豆包等）
  ├─ tools/            # AI 可调用的工具函数
  ├─ prompts/          # 系统提示词模板
  └─ __init__.py
src/cli/ai.py          # CLI 入口
```
