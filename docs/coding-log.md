...
## 2025-11-18 v0.2.1 延迟追踪修复

### 完成内容
- **修复关键问题**：更新 `SqliteDbHelper` 的初始 DDL，包含 v0.2.1 新增的三个延迟追踪字段
- **schema_version 升级**：从 `1` 升级到 `2`，支持新字段在初始化时创建
- **改进 Job 输出**：`confirm_trades` 区分"未到期跳过"与"超期延迟"
- **修复类型注解**：日报中 `list` 改为 `list[Trade]`
- **修复负天数显示**：待确认交易的剩余天数处理负值情况
- **修复文档示例**：Trade 类 docstring 使用小写状态标记

### 决策
- 采用 **Option A**（最稳定方案）：直接在初始 DDL 中包含新字段，避免新库初始化失败
- 新库创建时 `schema_version=2`，存量库通过迁移脚本升级（保持兼容）
- 所有状态标记统一使用小写（`normal` / `delayed` / `nav_missing`）

### 测试验证
- ✅ 新库初始化正常（包含延迟追踪字段）
- ✅ 延迟标记功能正常
- ✅ 自动恢复功能正常（NAV 补充后自动确认并清除延迟标记）
- ✅ 日报展示三类交易：已确认 / 待确认 / 异常延迟
