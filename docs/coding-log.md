...
## 2025-12-03 CLI 文件夹标准化重构

### 决策

统一 `src/cli/` 所有 15 个文件的代码结构，建立标准模式。

**核心原则**：
- 职责分离：`_parse_args()` / `_format_*()` / `_do_*()` / `main()`
- 数字标签注释：函数内部用 `# 1.` `# 2.` 标记步骤
- 统一日志：全部使用 `log()`，禁止 `print()`
- 标准返回码：0（成功）/ 4（参数错误）/ 5（其他失败）

**标准结构**（详见 `docs/architecture.md`）：
```python
def _parse_args() -> argparse.Namespace: ...
def _format_*(...) -> None: ...  # 格式化输出
def _do_*(...) -> int: ...       # 执行命令
def main() -> int: ...           # 路由入口
```
