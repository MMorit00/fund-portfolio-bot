    ...
    if fund_codes:
        ...
        for code in fund_codes:
            ...
            if fund:
                ...
                log(f"[Nav] ⚠️ 基金代码 {code} 未在系统中配置，跳过")
    else:
...
def fetch_missing_navs(
    *,
    ...
    if not missing_navs:
        log(f"[Nav] 扫描到 {len(delayed_trades)} 笔延迟交易，但所有 NAV 已存在，无需补抓")
        return FetchNavsResult(day=None, total=0, success=0, failed_codes=[])

    log(f"[Nav] 检测到 {len(missing_navs)} 个缺失 NAV，开始补抓...")
    ...
    for fund_code, pricing_date in sorted(missing_navs):
        if _fetch_single_nav(fund_code, pricing_date, fund_data_client, nav_repo):
            ...
            log(f"[Nav] 补抓失败：{fund_code} {pricing_date}")

    log(f"[Nav] 补抓完成：total={total} success={success} failed={len(failed_codes)}")
